...
TIMELINE_EXTENSION_NAMES = frozenset(
    {"timelineCacheRefs", "timelineRefs", "timelineList"}
)
...
class Snapshot:
    ...
    semantic_fingerprints: dict[str, Mapping[str, object]]
...
def audit(
    ...
    *,
    ...
    _run_audit_phase(
        ...
        lambda: _audit_relationship_preservation(
            before_snapshot, after_snapshot, issues
        ),
    )
    _run_audit_phase(
        progress_label,
    ...
    _run_audit_phase(
        ...
        lambda: _audit_content_type_preservation(
            before_snapshot, after_snapshot, issues
        ),
    )
    _run_audit_phase(
...
def _audit_dangling_relationships(
    snapshot_: Snapshot, issues: list[AuditIssue]
) -> None:
...
def _audit_table_integrity(
    ...
    _extend_new_issues(
        before.table_integrity_issues, after.table_integrity_issues, issues
    )
...
def _audit_chart_sheet_refs(
    ...
    _extend_new_issues(
        before.chart_sheet_ref_issues, after.chart_sheet_ref_issues, issues
    )
...
def _audit_pivot_sheet_refs(
    ...
    _extend_new_issues(
        before.pivot_sheet_ref_issues, after.pivot_sheet_ref_issues, issues
    )
...
def _extend_new_issues(
    ...
    issues.extend(
        issue for issue in after_issues if _issue_key(issue) not in before_keys
    )
...
def _semantic_drift_issue(
    feature: str,
    before_fingerprint: Mapping[str, object],
    after_fingerprint: Mapping[str, object],
    *,
    compact: bool,
) -> AuditIssue:
...
def _read_xml_parse_errors(archive: zipfile.ZipFile) -> list[tuple[str, str]]:
    ...
    if xml_cache is None:
        xml_cache = {}
        cast(Any, archive)._wolfxl_ooxml_xml_cache = xml_cache
    for part in sorted(
...
def _read_table_integrity_issues(archive: zipfile.ZipFile) -> list[AuditIssue]:
    issues: list[AuditIssue] = []
    for part in sorted(
        _feature_xml_parts(set(archive.namelist()), "xl/tables/", ".xml")
    ):
        root = _read_xml_or_none(archive, part)
        if root is None:
            continue
        ...
        if ref_width is not None and child_count != ref_width:
            mismatches.append(
                f"ref width={ref_width}, tableColumn children={child_count}"
            )
        if declared_count is not None and declared_count != child_count:
            mismatches.append(
                f"count={declared_count}, tableColumn children={child_count}"
            )
        if (
            declared_count is not None
            and ref_width is not None
            and declared_count != ref_width
        ):
...
def _formula_sheet_reference_names(formula: str) -> set[str]:
    ...
    for match in re.finditer(
        r"(?<![\]\w'])((?:[A-Za-z0-9_][A-Za-z0-9_ .:]*)!)", formula
    ):
...
def _read_semantic_fingerprints(
    ...
    *,
    ...
    def read_feature(
        name: str, func: Callable[[], FingerprintValue]
    ) -> FingerprintValue:
    ...
    read_feature(
        "data_validations", lambda: _data_validation_fingerprint(archive, parts)
    )
    ...
    read_feature(
        ...
        lambda: _xml_part_fingerprint(
            ...
            parts,
            tuple(
                sorted(part for part in parts if part.startswith("xl/namedSheetViews/"))
            ),
        ),
    )
    ...
    read_feature(
        "python", lambda: _xml_part_fingerprint(archive, parts, ("xl/python.xml",))
    )
    read_feature(
        "sheet_metadata",
    ...
    read_feature(
        "workbook_globals", lambda: _workbook_global_fingerprint(archive, parts)
    )
    return fingerprints
        ...
        out[part] = [
            ...
            (
                ...
                [
                    ...
                    for node in _nodes_by_local(root, "sheetView")
                ],
            ),
            (
                "protection",
            ...
            connections.append(
                (
                    ...
                    [
                        ...
                        for node in _nodes_by_local(connection, "extLst")
                    ],
                )
            )
        out[part] = [
...
def _style_theme_fingerprint(
    ...
    rels_by_owner = _relationships_by_owner(archive)
    for part in sorted(
        p for p in parts if p == "xl/styles.xml" or p.startswith("xl/theme/theme")
    ):
        if compact:
            payload = archive.read(part)
            out[part] = [
...
def _slicer_fingerprint(
    ...
    for part in sorted(_feature_xml_parts(parts, "xl/slicerCaches/", ".xml")):
        ...
        out[part] = [
            ...
            (
                "data",
                _stable_attrs(_first_node_by_local(root, "data"), ("pivotCacheId",)),
            ),
...
def _read_part_bytes_or_none(archive: zipfile.ZipFile, part: str) -> bytes | None:
    cache = cast(
        ...
        getattr(archive, "_wolfxl_ooxml_bytes_cache", None),
    )
    if cache is None:
        cache = {}
        cast(Any, archive)._wolfxl_ooxml_bytes_cache = cache
    if part in cache:
        return cache[part]
    try:
        payload = archive.read(part)
...
def _read_xml_or_none(
    archive: zipfile.ZipFile, part: str
) -> ElementTree.Element | None:
    cache = cast(
        dict[str, ElementTree.Element | None] | None,
        getattr(archive, "_wolfxl_ooxml_xml_cache", None),
    )
    if cache is None:
        cache = {}
        cast(Any, archive)._wolfxl_ooxml_xml_cache = cache
    if part in cache:
        return cache[part]
    payload = _read_part_bytes_or_none(archive, part)
    if payload is None:
    ...
    for node in root.iter():
        ...
        axes.append(
            (
                ...
                _stable_attrs(
                    _first_node_by_local(node, "numFmt"), ("formatCode", "sourceLinked")
                ),
...
def _manual_layouts(root: ElementTree.Element) -> list[object]:
    return [
        _xml_tree_fingerprint(node) for node in _nodes_by_local(root, "manualLayout")
    ]
    ...
    for part in sorted(_worksheet_parts(parts)):
        ...
        if external_only:
            formulas = [
                formula
                for formula in formulas
                if _is_external_workbook_formula(formula)
            ]
        if formulas:
            out[part] = formulas
    return out
...
def _is_structured_reference_formula(formula: str) -> bool:
    return (
        "[" in formula and "]" in formula and not _is_external_workbook_formula(formula)
    )


def _pivot_data_fields(
    root: ElementTree.Element,
...
def _pivot_calculated_fields(
    ...
    return [
        _stable_attrs(
            node, ("name", "formula", "hierarchy", "memberName", "mdx", "solveOrder")
        )
...
def _slicer_items(
    ...
    return [
        _stable_attrs(node, ("n", "c", "x", "s")) for node in _nodes_by_local(root, "i")
    ]
...
def _all_stable_attrs(node: ElementTree.Element | None) -> tuple[tuple[str, str], ...]:
    if node is None:
        return ()
    return tuple(
        sorted((_local_name(key), value) for key, value in node.attrib.items())
    )
...
def _snapshot_summary(snapshot_: Snapshot) -> dict[str, Any]:
    return {
        "path": snapshot_.path,
